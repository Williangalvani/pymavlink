...
This file is generated by a modified pymavlink found at https://github.com/Williangalvani/pymavlink/tree/logviewer
...
def generate_message_ids(outf, msgs, xml):
    ...
    outf.write(f"\n{get_mavhead(xml)}.nameMap = {{")
    for m in msgs:
        outf.write(f"\n    '{m.name.upper()}': {m.id},")
    outf.write("\n}\n")

    # generate Fields dicts
    outf.write(f"\n{get_mavhead(xml)}.messageFields = {{")
    for m in msgs:
        outf.write(f"\n    '{m.name.upper()}': [")
        for f in m.fields:
            outf.write(f"\n        '{f.name}',")
...
const MAVLinkSigning = function MAVLinkSigning(object){ 
...
const ${MAVPROCESSOR} = function(logger, srcSystem, srcComponent) {
...
${MAVPROCESSOR}.prototype.decode = function (msgbuf) {
    ...
    if (_.has(mavlink20.map, msgId) === false) {
    ...
    m.header = new mavlink20.header(msgId, mlen, seq, srcSystem, srcComponent)
    this.log(m)
    // console.log(msgbuf)
    return m
}
...
/* decode a buffer as a MAVLink message */
${MAVPROCESSOR}.prototype.checkIfValidMessage = function (msgbuf) {
    var magic, mlen, seq, srcSystem, srcComponent, unpacked, msgId, flag1, flag2, headerLen, payload
...
${MAVPROCESSOR}.prototype.preParse = function () {
    let i = 0
    let countv1 = 0
    let countv2 = 0
    ...
    for (let id of Object.keys(this.bufmap)) {
        for (let name of Object.keys(mavlink20.nameMap)) {
            if (mavlink20.nameMap[name] == id) {
                nameList.push(name)
            }
        }
    }
...
${MAVPROCESSOR}.prototype.parseType = function (type) {
    let messages = []
...
${MAVPROCESSOR}.prototype.trimFile = function (start, end) {
    let startOffset = null
    let endOffset = null
    let startTimestamp = null
    for (let offsets of this.bufmap[mavlink20.nameMap['HEARTBEAT']]) {
        let m = this.decode(this.buf.slice(offsets[0], offsets[1]))
        // The 8 bytes preceding the mavlink message are actually a very useful timestamp saved by the GCS!
        let slice = this.buf.slice(offsets[0]-8, offsets[0])
    ...
    if (endOffset === null) {
        const bufmap = this.bufmap[mavlink20.nameMap['HEARTBEAT']]
